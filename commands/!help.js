...
if(prefix === null) prefix = process.env.prefix;
  let color = db.get(`${process.env.owner}.color`) 
   if(color === null  ) color = process.env.color
    ...
    .setFooter(`${footer}・Prefix actuel : ${prefix}`)
...
.addField(`\`${prefix}join-settings\``, "Permet de paramétrer les actions à effectuer quand un membre rejoint le serveur")
.addField(`\`${prefix}leave-settings\``, "Permet de paramétrer les actions à effectuer quand un membre quitte le serveur")
...
.addField(`\`${prefix}backup list <serveur/emoji>\``, "Montre la liste des backups créé")
.addField(`\`${prefix}delete <serveur/emoji> backup <nom>\``, "Supprime une backup serveur/emoji grâce au nom")
.addField(`\`${prefix}load <serveur/emoji> backup <nom>\``, "Charge une backup emoji ou une backup serveur")
...
.addField(`\`${prefix}status <role id> <status>\``, "Permet de donner un rôle grâce au status (un seul serveur)")
.addField(`\`${prefix}status <on/off>\``, "Active ou désactive la commande status")
.addField(`\`${prefix}start <salon, durée, gagnants, prix>\``, "Lance un giveaway dans un salon")
.addField(`\`${prefix}end <message id>\``, "Termine un giveaway")
.addField(`\`${prefix}reroll <message id>\``, "Reroll un giveaway pour trouvé un autre gagnant")
.addField(`\`${prefix}setup\``, "Setup le système de giveaway")
.addField(`\`${prefix}new\``, "Créé un ticket")
.addField(`\`${prefix}add <membre>\``, "Ajouter quelqu'un au ticket")
.addField(`\`${prefix}remove <membre>\``, "Supprimer une personne du ticket")
.addField(`\`${prefix}close\``, "Fermer le ticket")
.addField(`\`${prefix}open\``, "Permet de rouvrir le ticket")
.addField(`\`${prefix}ticket-delete\``, "Supprimer un ticket ouvert")
...
.addField(`\`${prefix}reaction-role <emote> <messageid> <@role>\``, "Créé une réaction role")
.addField(`\`${prefix}reaction-role-remove <messageid> <emote>\``, "Supprime une réaction role")
...
const embedPages = [ mod, log, gestion,antiraid,bot,util];
createPages(client.interaction, message, embedPages, 60 * 1000, "blurple", "▶", "◀");

    }
    else if(db.get(`public_${message.guild.id}`) === true){
        const utilpublic = new Discord.MessageEmbed()
        .setTitle('Utilitaire')
        .setColor(color)
        .setDescription(`*Les paramètres peuvent être des noms, des mentions, ou des IDs\nSi ce ne sont pas des mentions ils doivent être séparés par \`,,\`*`)
        .addField(`\`${prefix}pic [membre]\``, "Permet de récupérer la photo de profil de quelqu'un")
        ...
        .addField(`\`${prefix}speed\``, "Affiche la vitesse de réaction du bot")
        .addField(`\`${prefix}channel [salon]\``, "Affiche les informations relatives à un salon")
        message.channel.send(utilpublic)
    }
}
}